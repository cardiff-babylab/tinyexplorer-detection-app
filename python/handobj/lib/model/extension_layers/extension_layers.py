...
class extension_layer(nn.Module):
    ...
    def forward(self, input, input_padded, roi_labels, box_info):
        ...
        loss_list = [self.hand_contactstate_part(input_padded, roi_labels, box_info), \
                    ...
                    self.hand_ownother_part(input, roi_labels, box_info)
                    ]
    ...
    def init_layers_weights(self):
        ...
        self.hand_ownother_layer = torch.nn.Linear(2048, 1)
        ...
        self.hand_ownother_loss = nn.BCEWithLogitsLoss()
    ...
    def hand_ownother_part(self, input, roi_labels, box_info):
        ownother_pred = self.hand_ownother_layer(input)  # [B, R, 1]
        ownother_loss = 0

        if self.training:
            for i in range(input.size(0)):
                gt_labels = box_info[i, :, 5]  # own/other label (0/1, or -1 for ignore)

                index = (roi_labels[i] == 2)
                index = index & (gt_labels >= 0)

                if index.sum() > 0:
                    targets = gt_labels[index].float().unsqueeze(-1)  
                    loss_sub = 0.1 * self.hand_ownother_loss(ownother_pred[i][index], targets)

                    if not ownother_loss:
                        ownother_loss = loss_sub
                    else:
                        ownother_loss += loss_sub

        return ownother_pred, ownother_loss
    ...
    def _init_weights(self):
        ...
        normal_init(self.hand_ownother_layer,0,0.01)
